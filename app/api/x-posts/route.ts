...
- hook: 1行目（最初の一文）。タイムラインでこの行だけ見えても続きを読みたくなる、単独で成立する掴み。蝶番か意外な断定を前に出す。説明調・前置きで始めない。
- post: X投稿本文。140字以内。必ず上記hookの一文で始める。続く文で蝶番を支え、読者が後で自分で確認できる「次に見るべき具体的な一点」を必ず含める。背景説明で字数を埋めない。
...
- 賛同・要約で終わらせず、あなたの横断的な視座から新しい角度を一つ足す。
...
JSONのみを返す。前後に説明文・コードフェンス・余計な文字を一切付けない。
{
  "core_question": "...",
  "candidates": [
    { "hinge": "...", "reversal": "...", "angle": "...", "hook": "...", "post": "...", "self_check": "..." }
  ],
  "trend_replies": [
    { "angle": "...", "reply": "...", "note": "..." }
  ]
